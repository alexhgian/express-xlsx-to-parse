...
exports.Mapper = function(Parse) {
    ...
    var CollectionMapper = function(conference, rowData, schema, callback) {
        ...
        **************************************/
        _.forEach(schema.boolean, function(val, key){
            if(rowData[val]){ // if not set the xlsx define value
                col.set(val, rowData[val].toLowerCase()=='yes' ||
                rowData[val]==true || rowData[val]=='true');
            }
        });
        ...
        **************************************/
        _.forEach(schema.primatives, function(val, key){
            ...
            } else if(rowData[val.name]){ // if not set the xlsx define value
                col.set(val.dbName || val.name, rowData[val.name]);
            }
        });
